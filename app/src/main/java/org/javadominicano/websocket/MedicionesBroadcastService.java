...
public class MedicionesBroadcastService {
    ...
    public void sendLatest() {
        ...
        m.setTemperatura(repoTemperatura.findTopByOrderByFechaDesc(pr).get(0).getTemperatura());
        m.setHumedad(repoHumedad.findTopByOrderByFechaDesc(pr).get(0).getHumedad());
        m.setVelocidadViento(repoVelocidad.findTopByOrderByFechaDesc(pr).get(0).getVelocidad());
        m.setDireccionViento(repoDireccion.findTopByOrderByFechaDesc(pr).get(0).getDireccion());
        m.setPrecipitacion(repoPrecipitacion.findTopByOrderByFechaDesc(pr).get(0).getProbabilidad());
        m.setPresion(repoPresion.findTopByOrderByFechaDesc(pr).get(0).getPresion());
        m.setHumedadSuelo(repoHumedadSuelo.findTopByOrderByFechaDesc(pr).get(0).getHumedad());
